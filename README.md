...
Before running the build, copy the example sequences into the `data/` directory like so:

```bash
mkdir -p data/ha
cp example_data/metadata_h5n1.tsv data/metadata.tsv
cp example_data/sequences_h5n1_ha.fasta data/ha/sequences.fasta
```

Then run the the build. If you'd like to consistently run your own data, then you can place your fasta file in `data`. Alternatively, you can alter the `Snakefile` to remove references to S3 and add paths to your own files (see rules `download_sequences` and `download_metadata`).
